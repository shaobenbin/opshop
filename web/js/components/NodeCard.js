(function() {
	const css = `
        ...
        .username-text { font-size: 16px; font-weight: bold; color: var(--text-main); letter-spacing: 0.3px; }

        /* 认证标签去突兀化 */
        .badge-auth-subtle {
            font-size: 9px; padding: 1px 4px; border-radius: 3px;
            border: 1px solid rgba(255,255,255,0.1); color: var(--text-dim);
            font-weight: normal; background: rgba(0,0,0,0.2);
        }
        ...
        .app-item { background: #18191c; border: 1px solid #2d2f33; padding: 16px; border-radius: 6px; cursor: grab; position: relative; border-left: 3px solid var(--jb-green); }
        .app-item:hover { border-color: var(--jb-green); background: #1e2024; }

        .app-tab-group { display: flex; gap: 2px; margin-bottom: 8px; }
        .app-tab { font-size: 9px; font-weight: bold; padding: 1px 6px; border-radius: 2px; }
        ...
        <div class="node-header" @dblclick="$emit('edit-node', node)">
            <div class="node-info-main">
                <div class="node-meta-tabs">
                    ...
                    <div class="meta-tab provider">{{ node.provider || 'LOCAL' }}</div>
                </div>
        ...
        <div v-for="user in node.users" :key="user.id" class="user-section">
            <div class="user-header">
                <div class="user-info">
                    ...
                    <span class="badge-auth-subtle">
                        {{ user.password ? 'PWD_AUTH' : 'SSH_KEY' }}
                    </span>
                </div>
                ...
                <template #item="{element}">
                    <div class="app-item" @dblclick.stop="$emit('edit-app', {nodeId: node.id, userId: user.id, app: element})">
                        ...
                        <div class="app-tab-group">
                            <span v-if="element.lang" class="app-tab lang">{{ element.lang }}</span>
                            <span v-if="element.app_type" class="app-tab type">{{ element.app_type }}</span>
                        </div>

                        <div style="font-size: 11px; color: var(--jb-orange); font-weight: 600; margin-bottom: 4px;">:: {{ element.project || 'SYS' }}</div>

                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            <div style="font-size: 15px; font-weight: bold; color: #fff;">{{ element.name }}</div>
                            <div v-if="element.port" style="color: var(--jb-green); font-size: 12px; font-family: monospace;">#{{ element.port }}</div>
                        </div>

                        <div v-if="element.deploy_path" style="font-size: 11px; color: #6a8759; margin-top: 8px; font-family: monospace; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                            PWD: {{ element.deploy_path }}
                        </div>
                    </div>
                </template>
            </draggable>
            <div style="padding: 0 18px 18px;"><button class="btn-add-app" @click="$emit('add-app', {nodeId: node.id, userId: user.id})">+ {{ t('add_app') }}</button></div>
        </div>

        <div class="card-footer">
            <button class="btn-add-user" @click="$emit('add-user', node.id)">
                + ADD USER ACCOUNT
            </button>
        </div>
    </div>
